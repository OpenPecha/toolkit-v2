...
### `add_layer(self, base_name: str, layer_type: AnnotationType)`

- Add a new annotation layer for a given base.
- Parameters:
  - `base_name` (str): The name of the base file to associate with this layer.
  - `layer_type` (AnnotationType): The type of annotation layer.
- Returns: `AnnotationStore`.

### `check_annotation(self, annotation: Dict, layer_type: AnnotationType)`

- Validates the format of the annotation data.
- Parameters:
  - `annotation` (Dict): The annotation data.
  - `layer_type` (AnnotationType): The type of annotation.
- Raises `ValueError` if validation fails.

### `add_annotation(self, ann_store: AnnotationStore, annotation: Dict, layer_type: AnnotationType)`
...
### `get_layer(self, basefile_name: str, annotation_type: AnnotationType)`
...
# Add a layer
layer, layer_path = pecha.add_layer(base_name, AnnotationType.chapter)
...
pecha.add_annotation(layer, annotation, AnnotationType.chapter)
...
This will add a new annotation layer for the specified base text. The `layer` attribute will be an `AnnotationStore` object with the `AnnotationType.chapter` as the layer type. When creating any annotation, the annotation must be associated with a layer where its layer type is mentioned in the `openpecha.pecha.layer.AnnotationType` class.

A particular pecha can have more than one layer with same layer type. For example, a pecha can have multiple `AnnotationType.segment` layers. So to differentiate when adding any layer, its final name will be `layer_type` and '-' followed by three randomly generated number.
