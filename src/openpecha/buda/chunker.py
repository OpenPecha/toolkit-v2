...
class EnglishEasyChunker(StringChunker):
    def __init__(self, full_string, target_len, start_cc=0, end_cc=0):
        StringChunker.__init__(
            self, full_string, ENG_PATTERN, target_len, start_cc, end_cc
        )


TIB_PATTERN = re.compile(
    r"([སའངགདནབམརལཏ]ོ[་༌]?[།-༔][^ཀ-ཬ]*|(།།|[༎-༒])[^ཀ-ཬ༠-༩]*[།-༔][^ཀ-ཬ༠-༩]*)"
)
...
class TibetanEasyChunker(StringChunker):
    def __init__(self, full_string, target_len, start_cc=0, end_cc=0):
        StringChunker.__init__(
            self, full_string, TIB_PATTERN, target_len, start_cc, end_cc
        )
