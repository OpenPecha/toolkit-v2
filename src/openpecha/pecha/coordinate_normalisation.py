...
class CoordinateNormalisation():
    ...
    def update_metadata(self, pecha_id, dict_data):
        """
        ...
        if dict_key in pecha_metadata.source_metadata:
            pecha_metadata.source_metadata[dict_key].append(
                dict_data.value()
            )
        else:
    ...
    def transfer_layer(self):
        ...
        self.get_layer_name()
        target_pecha.merge_pecha(source_pecha, self.source_base_name, self.target_base_name)
        self.update_metadata(
            pecha_id=self.target_pecha_path.name,
            dict_data={
    ...
    def normalise_coordinate(self):
        ...
        for layer in self.target_layer:
            for ann in layer[1]:
                ...
                curr_ann = {}
        transfered_to_display_map = self.map_display_to_transfer(display_layer_ann, transfered_layer_ann)
    ...
    def map_display_to_transfer(self, display_layer_ann, transfer_layer_ann):
        ...
        """
        transfered_to_display_map = {}
        for transfer_key, transfer_span in transfer_layer_ann.items():
            t_start, t_end = transfer_span['Span']['start'], transfer_span['Span']['end']
            transfered_to_display_map[transfer_key] = []
            for display_key, display_span in display_layer_ann.items():
                d_start, d_end = display_span['Span']['start'], display_span['Span']['end']
                if t_start <= d_start <= t_end or t_start <= d_end <= t_end:
                    transfered_to_display_map[transfer_key].append([display_key, [d_start, d_end]])
    ...
    def create_display_translation_alignment_layer(self, transfered_to_display):
        ...
        translation_pecha = StamPecha(self.translation_pecha_path)
        self.translation_layer = translation_pecha.get_layers(self.translation_base_name)
        ...
        for alignment_key, display_info in transfered_to_display.items():
            start, end = translation_layer_ann[alignment_key]["Span"]["start"], translation_layer_ann[alignment_key]["Span"]["end"]
    ...
    def write_layer(self):
        ...
        """
        pecha = Pecha(pecha_id=self.translation_pecha_id, pecha_path=self.translation_pecha_path)
        segment, segment_path = pecha.add_layer(self.translation_base_name, LayerEnum.pecha_display_alignment_segment)
        for root_id, info in self.alignment_data.items():
            ...
            pecha.add_annotation(segment, segment_ann, LayerEnum.pecha_display_alignment_segment)
        segment.save()

        self.update_metadata(
            pecha_id=self.translation_pecha_id,
            dict_data={
