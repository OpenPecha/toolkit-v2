...
class PechaBaseUpdate:
    ...
    def update_index_layer(self):
        ...
        for (_id, src_ann), (_id, dst_ann) in zip(
            src_index_layer["annotations"].items(),
            dst_index_layer["annotations"].items(),
        ):
            ...
            for (_, src_sub_text), (_, dst_sub_text) in zip(
                src_ann["parts"].items(), dst_ann["parts"].items()
            ):
