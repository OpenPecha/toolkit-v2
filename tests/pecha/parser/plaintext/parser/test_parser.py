...
def test_chapter_parser_pipeline():
    ...
    assert openpecha_framework.data["raw_string"] == expected_raw_string
...
def test_pecha_convertor_pipeline():
    DATA = Path(__file__).parent / "data"
    input_file = DATA / "basefile.txt"
    input_text = input_file.read_text(encoding="utf-8")

    openpecha_framework = PechaFrameWork(input_text)
    openpecha_framework.data = {
        "raw_string": expected_raw_string,
        "Chapter": expected_chapter_output,
        "Tsawa": expected_tsawa_output,
    }
    ...
    for pecha_ann_file in pecha_ann_file_paths:
        assert pecha_ann_file.exists
    """ clean up """
    rmtree(output_path)


test_pecha_convertor_pipeline()
