...
from openpecha.utils import load_yaml, load_json
                    ...
                    for d in dataset["data"]:
                        if "key" in d and "value" in d:
                            if isinstance(d["value"], dict) and "value" in d["value"]:
                                data_map[d["@id"]] = {"key": d["key"], "value": d["value"]["value"]}
                            else:
                                data_map[d["@id"]] = {"key": d["key"], "value": d["value"]}
...
def get_annotation_bounds(annotation):
    ...
    start = (offset.get("begin", {}).get("value", None) 
            if isinstance(offset.get("begin"), dict) 
            else offset.get("start"))
    end = (offset.get("end", {}).get("value", None)
...
def test_pagination_layer(pecha, base_name, expected_pagination_layer_dict):
    ...
    _, pagination_layer_file = pecha.get_layer_by_ann_type(base_name, LayerEnum.pagination)
    assert pagination_layer_file.exists(), "Pagination layer file should exist"
    assert pagination_layer_file.name.startswith("Pagination-"), "Pagination layer file name should start with 'Pagination-'"
    
    pagination_layer_content = json.loads(pagination_layer_file.read_text())
    
    for i, (actual, expected) in enumerate(zip(pagination_layer_content["annotations"], expected_pagination_layer_dict["annotations"])):
        # Compare spans
        actual_start, actual_end = get_annotation_bounds(actual)
        expected_start, expected_end = get_annotation_bounds(expected)
        
        assert actual_start == expected_start, f"Pagination start position mismatch at index {i}"
        assert actual_end == expected_end, f"Pagination end position mismatch at index {i}"
        
        # Compare data
        actual_data = extract_annotation_values(pagination_layer_content, actual)
        expected_data = extract_annotation_values(expected_pagination_layer_dict, expected)
        
        assert actual_data.get("imgnum") == expected_data.get("imgnum"), f"Image number mismatch at index {i}"
        assert actual_data.get("reference") == expected_data.get("reference"), f"Reference mismatch at index {i}"
...
def test_language_layer(pecha, base_name, expected_language_layer_dict):
    ...
    assert language_layer_file.name.startswith("Language-"), "Language layer file name should start with 'Language-'"
    
    language_layer_content = json.loads(language_layer_file.read_text())
    
    for i, (actual, expected) in enumerate(zip(language_layer_content["annotations"], expected_language_layer_dict["annotations"])):
        # Compare spans
        actual_start, actual_end = get_annotation_bounds(actual)
        expected_start, expected_end = get_annotation_bounds(expected)
        
        assert actual_start == expected_start, f"Language start position mismatch at index {i}"
        assert actual_end == expected_end, f"Language end position mismatch at index {i}"
        
        # Compare data
        actual_data = extract_annotation_values(language_layer_content, actual)
        expected_data = extract_annotation_values(expected_language_layer_dict, expected)
        
        assert actual_data.get("language") == expected_data.get("language"), f"Language value mismatch at index {i}"

def test_confidence_layer(pecha, base_name, expected_confidence_layer_dict):
    """Test OCR confidence layer annotations"""
    _, confidence_layer_file = pecha.get_layer_by_ann_type(base_name, LayerEnum.ocr_confidence)
    assert confidence_layer_file.exists(), "OCR confidence layer file should exist"
    assert confidence_layer_file.name.startswith("OCRConfidence-"), "OCR confidence layer file name should start with 'OCRConfidence-'"
    
    confidence_layer_content = json.loads(confidence_layer_file.read_text())
    
    for i, (actual, expected) in enumerate(zip(confidence_layer_content["annotations"], expected_confidence_layer_dict["annotations"])):
        # Compare spans
        actual_start, actual_end = get_annotation_bounds(actual)
        expected_start, expected_end = get_annotation_bounds(expected)
        
        assert actual_start == expected_start, f"Confidence start position mismatch at index {i}"
        assert actual_end == expected_end, f"Confidence end position mismatch at index {i}"
        
        # Compare data
        actual_data = extract_annotation_values(confidence_layer_content, actual)
        expected_data = extract_annotation_values(expected_confidence_layer_dict, expected)
        
        assert actual_data.get("confidence") == expected_data.get("confidence"), f"Confidence value mismatch at index {i}"
...
def test_build_layers():
    ...
    expected_pagination_layer_dict = load_json(
        Path(__file__).parent / "data" / "file_per_page" / "pecha_opf_expected_data" / "expected_Pagination.json"
    )
    expected_language_layer_dict = load_json(
        Path(__file__).parent / "data" / "file_per_page" / "pecha_opf_expected_data" / "expected_Language.json"
    )
    expected_confidence_layer_dict = load_json(
        Path(__file__).parent / "data" / "file_per_page" / "pecha_opf_expected_data" / "expected_OCRConfidence.json"
    )
    ...
    opf_options = {
        ...
        "max_low_conf_per_page": 50
    }

    # Create pecha and test layers
    with tempfile.TemporaryDirectory() as tmpdirname:
        formatter = HOCRFormatter(output_path=tmpdirname)
        pecha = formatter.create_pecha(data_provider, pecha_id, opf_options, ocr_import_info)
    ...
    print("✅ All layer assertions passed!")
